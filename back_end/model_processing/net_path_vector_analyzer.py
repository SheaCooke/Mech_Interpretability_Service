...
class Net_Path_Vector_Analyzer:
    ...
    def _get_id_mapping(self, inf_results: list[dict]) -> np.ndarray:
    ...
    def _get_distance_matrix(self, act_matrix: np.ndarray) -> np.ndarray:
    ...
    def find_all_similar_pairs(self, inference_results: list[dict], low: float = 0.0, high: float = 0.2, filter: PredictionFilter = PredictionFilter.ALL) -> list[dict]:
        ...
        if self.distance_matrix is None:
            self.distance_matrix = self._get_distance_matrix(self.activation_matrix)
    ...
    def get_cluster_plot_data(self, inference_results, filter: PredictionFilter = PredictionFilter.ALL) -> dict:
        """
        reduces activation vectors to 2D using UMAP
        ...
        return {'method': 'UMAP','points': points}
